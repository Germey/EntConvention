...
//}
     ...
     * @param string $app_id 微信公众号应用唯一标识
     * @param string $redirect_url 授权后重定向的回调链接地址，重定向后此地址将带有授权code参数，
     *                      该地址的域名需在微信公众号平台上进行设置，
     *                      步骤为：登陆微信公众号平台 => 开发者中心 => 网页授权获取用户基本信息 => 修改
     * @param string $state 推广参数
     ...
     */
    public static function createOauthUrlForCode($app_id, $redirect_url, $state ='STATE',$more_info = false)
    {
        $urlObj = array();
        $urlObj['appid'] = $app_id;
